...
- **동타(같은 시각 타일) 보정(`--min-gap`)**: 같은 타이밍에 몰린 타일을 같은 렌더 프레임에 같이 누르면 게임이 한 번만 인식하므로, 입력 사이에 최소 간격(기본 16ms)을 둬 각 입력이 별도 프레임에 들어가게 함
...
# 동타(같은 시각 타일) 보정 간격 조정 (ms, 기본 16)
python main.py --min-gap 10        # 144Hz 등 고주사율이면 줄여도 됨
python main.py --min-gap 0         # 보정 끄기(동타가 한 번만 인식될 수 있음)
